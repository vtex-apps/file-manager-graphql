...
- **Decision**: This app's `manifest.json` declares the resource policy `vtex.file-manager` publishes for `/policies/*`: `file-manager-bucket-config-rw`, VRN-scoped to `vrn:vtex.file-manager:{{region}}:{{account}}:{{workspace}}:/policies/*` (confirmed published resource, LicenseManager `resourceKey: file-manager-bucket-config`, category `Infrastructure`). **Name this concretely at spec time** — an earlier version of this decision deferred the exact name to "whatever file-manager publishes once it exists"; that ambiguity is exactly what let the route path itself drift during implementation (a route rename was tried as a workaround for an unrelated bucket-naming collision, then reverted — see file-manager's own spec, US-1's router-collision rule). Specifying the concrete VRN/resource name up front removes the temptation to change the route path instead of fixing the actual naming collision.
- **Consequences**: US-2/US-3 require both (a) file-manager's `policies.json` publishing `file-manager-bucket-config-rw` and (b) this app's `manifest.json` declaring it, before any `/policies/*` call reaches file-manager's controller. This is a hard, blocking cross-repo dependency distinct from, and in addition to, the existing LicenseManager-based authorization already documented in Decision 5.
...
3. **Cross-repo prerequisite (blocks US-2/US-3)** — `vtex.file-manager` publishes `file-manager-bucket-config-rw` covering `/policies/*` (Decision 6, resource confirmed published). This app's `manifest.json` must declare it before any `/policies/*` call can reach file-manager's controller in a real environment — implementation and unit tests with mocks can proceed in parallel, but end-to-end testing of US-2/US-3 is sequenced after this declaration.
